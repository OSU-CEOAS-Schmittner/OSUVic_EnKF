...
from EnKF_2fwf import EnKF2fwf
from write_files import write_files
from write_kalman import write_kalman
from fwf_change import fwf_2change
from fwf_file import fwf_2file
from run_done import run_done
from write_control import write_control
from fwf_merge import fwf_2merge
from d13C import d13C, d13Cobs
from fwf_kalman import fwf_2kalman
from merge_stats import merge_2stats
from string_yr import string_yr
from rmse_fwf import rmse
...
execname = 'fwfnobio.q'
runname = '2fwf0_0.2infl0.05kalpi500yr_r5_q1_tavgint_50runstep50slopeslsigma5tempsursigma1_nature10915data'
...
def exe(n,tempsigma,slsigma,fwf_init1,fwf_init2,fwf_rate,slstart,randamp1,randamp2,initage, startyear,runtime,runstep,tsiint,tavgint,tavgper,tavgfile,lvar0,lvar1,dirpath,ensname,datadir,execname,runname):
	output = 'pr_' + runname
	...
	os.system('rm ' + dirpath + '/' + ensname + '*/random1.dat')
	os.system('rm ' + dirpath + '/' + ensname + '*/random2.dat')
	os.system('rm ' + dirpath + '/' + ensname + '*/tsi' + runname + '.nc') 
	os.system('rm ' + dirpath + '/' + ensname + '*/tavg' + runname + '.nc')
	os.system('rm ' + dirpath + '/' + ensname + '*/randamp1.dat')
	os.system('rm ' + dirpath + '/' + ensname + '*/randamp2.dat')
	os.system('rm ' + dirpath + '/kal_hist' + runname + '.npz')

	for i in range(n):
		np.savetxt(dirpath + '/' + ensname + str(i) + '/randamp1.dat',np.array([randamp1]),fmt = '%10.8f')
		np.savetxt(dirpath + '/' + ensname + str(i) + '/randamp2.dat',np.array([randamp2]),fmt = '%10.8f')
	...
	for i in range(startyear,startyear+runtime,runstep):
		...
		fwfinit1,fwfinit2,fwfrate = fwf_2change(n,fwf_init1,fwf_init2,fwf_rate,dirpath,ensname,runstep,step,init_sealev)

		if step > 0:
			fwfinit1new,fwfinit2new = EnKF2fwf(tempsigma,slsigma,randamp1,randamp2,slstart,n,dirpath,ensname,datadir,runname,yr,agerel,runstep,step,tavgint,tavgfile)
			f.write('EnKF done\n')
			fwf_2kalman(n,fwfinit1,fwfinit2,fwfinit1new,fwfinit2new,randamp1,randamp2,dirpath,step,runstep)
			fwfinit1 = fwfinit1new
			fwfinit2 = fwfinit2new
		else:
			fwf_2kalman(n,fwfinit1,fwfinit2,fwfinit1,fwfinit2,randamp1,randamp2,dirpath,step,runstep)

		
		fwf_2file(n,fwfinit1,fwfinit2,fwfrate,dirpath,ensname,i,runstep)
	...
	fwf_2change(n,fwf_init1,fwf_init2,fwf_rate,dirpath,ensname,runstep,step,init_sealev)
	...
	fwf_2merge(n,dirpath,ensname,runtime,runstep,runname,tsiint)
	os.system('nces ' + dirpath + '/' + ensname + '*/' + 'tsi' + runname + '.nc ' + dirpath + '/tsi' + runname + 'stat.nc')
	merge_2stats(n,dirpath,ensname,runname)
	rmse(n,dirpath,ensname,runname,datadir+'/'+tavgfile,istempsur = True,isc14 = False,isd13c = False)
	f.write('The Kalman Filter has done its work!\n')
	f.close()
	return ('The Kalman Filter has done its work!')

exe(n,tempsigma,slsigma,fwf_init1,fwf_init2,fwf_rate,slstart,randamp1,randamp2,initage, startyear,runtime,runstep,tsiint,tavgint,tavgper,tavgfile,lvar0,lvar1,dirpath,ensname,datadir,execname,runname)
